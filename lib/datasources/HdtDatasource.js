...
function HdtDatasource(options) {
  ...
  if (options.memento)
    this._mementos = getMementoList(hdtFile);
...
HdtDatasource.prototype.getMementoMap = function () {
...
function getMementoList(file) {
  // Get list of mementos from filesystem
  ...
  var mementos = {};
  files = _.without(files, path.basename(file));
  files.forEach(function(item) {
    var base = path.basename(item, ext);
    if (base.indexOf(filebase) > -1) {
      try {
        var datetime = parseInt(base.split('-')[1]);
        mementos[datetime] = new ExternalHdtDatasource({file: path.join(dir, item)});
      } catch (err){
        console.log(err);
      }
    }
